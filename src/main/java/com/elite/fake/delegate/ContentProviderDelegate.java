...
public class ContentProviderDelegate {
    ...
    public static void update(Object holder, String auth) {
        ...
        switch (auth) {
            case "settings":
                bContentProvider = new SystemProviderStub().wrapper(iInterface, EliteInstaller.getHostPkg());
                break;
            default:
                bContentProvider = new ContentProviderStub().wrapper(iInterface, EliteInstaller.getHostPkg());
                break;
        }
        if (BuildCompat.isOreo()) {
    ...
    public static void init() {
        ...
        EliteInstaller.getContext().getContentResolver().call(Uri.parse("content://settings"), "", null, null);
        Object activityThread = EliteInstaller.mainThread();
        ...
        for (Object value : map.values()) {
            ...
            if (!sInjected.contains(providerName)) {
                ...
                BRActivityThreadProviderClientRecordP.get(value)._set_mProvider(new ContentProviderStub().wrapper(iInterface, EliteInstaller.getHostPkg()));
