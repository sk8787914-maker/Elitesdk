    ...
    */
    ...
    private Boolean handleLaunchActivity(Object client) {
        ...
        if (activityInfo != null) {
            ...
            EliteInstaller.getBActivityManager().onActivityCreated(taskId, token, stubRecord.mActivityRecord);

            if (BuildCompat.isS()) {
                Object record = BRActivityThread.get(EliteInstaller.mainThread()).getLaunchingActivity(token);
                if (record != null) {
    ...
    private boolean handleCreateService(Object data) {
        if (BActivityThread.getAppConfig() != null) {
            ...
            if (!serviceInfo.name.equals(ProxyManifest.getProxyService(BActivityThread.getAppPid()))
                ...
                EliteInstaller.getBActivityManager().startService(intent, null, false, BActivityThread.getUserId());
                return true;
            }
        }
        return false;
