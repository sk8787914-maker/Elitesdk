...
import com.elite.core.system.pm.IBPackageManagerService;
...
public class BPackageManager extends BlackManager<IBPackageManagerService> {
    ...
    public Intent getLaunchIntentForPackage(String packageName, int userId) {
		...
		List<ResolveInfo> ris = queryIntentActivities(intentToResolve,0,intentToResolve.resolveTypeIfNeeded(EliteInstaller.getContext().getContentResolver()),userId);
		if (ris == null || ris.size() == 0) {
			intentToResolve.removeCategory(Intent.CATEGORY_INFO);
			intentToResolve.addCategory(Intent.CATEGORY_LAUNCHER);
			intentToResolve.setPackage(packageName);
			ris = queryIntentActivities(intentToResolve,0,intentToResolve.resolveTypeIfNeeded(EliteInstaller.getContext().getContentResolver()),userId);
		}
