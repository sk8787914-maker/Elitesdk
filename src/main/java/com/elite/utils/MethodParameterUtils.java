...
public class MethodParameterUtils {
    ...
    public static String replaceFirstAppPkg(Object[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            if (args[i] instanceof String) {
                ...
                if (EliteInstaller.get().isInstalled(value, BActivityThread.getUserId())) {
                    args[i] = EliteInstaller.getHostPkg();
                    return value;
                }
            }
        }
    ...
    public static String replaceLastAppPkg(Object[] args) {
        ...
        if (index != -1) {
            ...
            if (EliteInstaller.get().isInstalled(pkg, BActivityThread.getUserId())) {
                args[index] = EliteInstaller.getHostPkg();
            }
            return pkg;
        }
        return null;
    ...
    public static void replaceLastUserId(Object[] args) {
        ...
        if (index >= 0 && args[index] instanceof Integer) {
            args[index] = EliteInstaller.getHostUserId();
        }
    }
